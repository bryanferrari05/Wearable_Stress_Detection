...
from validate_resp_chest_csv import FEATURE_COLUMNS as FEATURE_COLS


DATASET_FILE = Path("data_features") / "resp_chest_features_all_60s.csv"
RESULTS_DIR = Path("results")
PER_SUBJECT_FILE = RESULTS_DIR / "resp_chest_rf_loso_per_subject.csv"
FEATURE_IMPORTANCE_FILE = RESULTS_DIR / "resp_chest_rf_feature_importance.csv"
SUMMARY_FILE = RESULTS_DIR / "resp_chest_rf_summary.txt"
...
def load_dataset():
    if not DATASET_FILE.exists():
        raise FileNotFoundError(
            ...
            "Prima esegui src/extract_resp_chest_features_all.py."
        )
...
def validate_dataset(df):
    ...
    if not finite_features.all():
        raise ValueError("Sono presenti NaN o valori infiniti nelle feature RESP chest.")
...
def build_summary(
    ...
    lines = [
        "# RESP chest + Random Forest LOSO summary",
        "",
        ...
        feature_importance_summary_df.head(20).to_string(index=False),
        "",
    ...
    lines.extend(
        [
            "",
            "## Nota",
            "Questa e' la baseline RESP chest con Random Forest. Il modello usa "
            "solo feature estratte dal segnale respiratorio chest e non include "
            "ancora tuning degli iperparametri.",
        ]
    )
    return "\n".join(lines) + "\n"
...
def update_progress(summary_values):
    ...
    section_title = "## Passo 17 - RESP chest + Random Forest LOSO"
