...
PER_SUBJECT_FILE = RESULTS_DIR / "bvp_knn_top15_loso_per_subject.csv"
FEATURE_IMPORTANCE_FILE = RESULTS_DIR / "bvp_knn_top15_feature_importance.csv"
SUMMARY_FILE = RESULTS_DIR / "bvp_knn_top15_summary.txt"
...
def load_dataset():
    if not DATASET_FILE.exists():
        raise FileNotFoundError(
            ...
            "Prima esegui src/extract_bvp_features_all.py."
        )
...
def validate_dataset(df):
    ...
    if not np.isfinite(df[FEATURE_COLS].to_numpy(dtype=float)).all():
        raise ValueError("Sono presenti NaN o valori infiniti nelle feature BVP.")
...
def run_loso_knn_top15(df):
    ...
    for train_idx, test_idx in logo.split(df[FEATURE_COLS], y, groups):
        ...
        importance_df = rank_features(X_train_all, y_train)
        ...
        X_train = df.iloc[train_idx][TOP15_FEATURE_COLS]
        X_test = df.iloc[test_idx][TOP15_FEATURE_COLS]
...
def build_summary(
    ...
    lines = [
        "# BVP wrist + kNN top15 LOSO summary",
        "",
        f"Dataset usato: {DATASET_FILE}",
        f"Feature candidate ({len(FEATURE_COLS)}): {', '.join(FEATURE_COLS)}",
        ...
        "",
        "## Confronto",
        f"BVP completo 25 feature: F1={REFERENCE_METRICS['bvp_full_25']['f1']:.4f} "
        f"({format_percent(REFERENCE_METRICS['bvp_full_25']['f1'])}), "
        f"Accuracy={REFERENCE_METRICS['bvp_full_25']['accuracy']:.4f} "
        f"({format_percent(REFERENCE_METRICS['bvp_full_25']['accuracy'])})",
        f"Delta F1 top15 - 25 feature: "
        f"{summary_values['aggregate_f1'] - REFERENCE_METRICS['bvp_full_25']['f1']:+.4f} "
        f"({format_percent(summary_values['aggregate_f1'] - REFERENCE_METRICS['bvp_full_25']['f1'])})",
        "",
    ...
    lines.extend(
        [
            "",
            "## Interpretazione",
            "La versione top15 riduce le feature BVP da 25 a 15 e migliora il F1 "
            "rispetto alla baseline completa. Le feature selezionate sono soprattutto "
            "HR, IBI, RMSSD/SDNN, conteggio picchi e prominenza, quindi sono anche "
            "piu' facili da motivare fisiologicamente.",
        ]
    )
...
def update_progress(summary_values):
    ...
    section_title = "## Test BVP top15 feature"
...
- Top 15 feature aggregate: `{", ".join(TOP15_FEATURE_COLS)}`
- Risultati:
...
- Delta F1 vs BVP completo 25 feature: {summary_values["aggregate_f1"] - REFERENCE_METRICS["bvp_full_25"]["f1"]:+.4f} ({format_percent(summary_values["aggregate_f1"] - REFERENCE_METRICS["bvp_full_25"]["f1"])})
...
def main():
    df = load_dataset()
    validate_dataset(df)
    per_subject_df, y_true, y_pred, feature_importance_df = run_loso_knn_top15(df)
    summary_values = save_results(df, per_subject_df, feature_importance_df, y_true, y_pred)
